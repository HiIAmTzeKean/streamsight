...
def empty_matrix(session_vars: dict) -> InteractionMatrix:
    ...
    df = pd.DataFrame(columns=[session_vars["TIMESTAMP_IX"], session_vars["USER_IX"], session_vars["ITEM_IX"], "rating"])
    return InteractionMatrix(df, session_vars["ITEM_IX"], session_vars["USER_IX"], session_vars["TIMESTAMP_IX"])
...
def matrix_no_timestamps(test_dataframe: pd.DataFrame, session_vars: Dict[str, Any]) -> InteractionMatrix:
    ...
    im = InteractionMatrix(df, session_vars["ITEM_IX"], session_vars["USER_IX"], "dummy", skip_df_processing=True)
